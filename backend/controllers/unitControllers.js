...
export const addUnit = async (req, res) => {
  try {
    const {name, movement, toughness, save, wounds, leadership, OC, cost} = req.body

  if (!name || !movement || !toughness || !save || !wounds || !leadership) {
      return res.status(400).json({success: false, message: "Missing data!"})
    }

    
    const profile = await sql.query(`INSERT INTO simple_profile (name, movement, toughness, save, wounds, leadership, OC, cost)
    VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [name, movement, toughness, save, wounds, leadership, OC, cost])
...
export const updateUnit = async (req, res) => {
  try {
    ...
    const unit = await sql.query(`UPDATE simple_profile 
      SET name = $1, movement = $2, toughness = $3, save = $4, wounds = $5, leadership = $6, OC = $7, cost = $8 
      WHERE id = $9`, [name, movement, toughness, save, wounds, leadership, OC, cost, id])
